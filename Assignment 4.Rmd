...
```
  ...
  Ans:-  Coefficient value of the income and balance  2.080898e-05 and 5.647103e-03.
...
```
Ans: The observation is correctly classified 
```{r}
Weekly[1,]$Direction
```

  Ans:-  The result is true and the direction is Down so the first value is wrong
...
```
  ...
  Ans:-  all values column is true and the error row value has changed to 0 and 1 by refering it as true or false.
